...
def main() -> int:
    ...
    ap.add_argument(
        ...
        default=os.getenv("SUPERVISOR_AGENT", cfg.agent_default),
        help="Model CLI used for supervisor loops (auto: prefer Claude, fallback Codex).",
    )
    ap.add_argument("--agent-role", type=str, default=os.getenv("SUPERVISOR_AGENT_ROLE", ""))
    ap.add_argument("--agent-prompt", type=str, default=os.getenv("SUPERVISOR_AGENT_PROMPT", ""))
    ...
    if not args.sync_via_git:
        ...
        for _ in range(args.sync_loops):
            ...
            iter_log_path = log_file("supervisor-legacy-")
            logp = make_logger(iter_log_path)
            try:
                prompt_file, _, selected_prompt = _select_prompt(router_cmd, logp, iteration_override=_ + 1)
            except Exception as e:
                logp(f"ERROR: {e}")
            ...
            if not prompt_file.exists():
                ...
                return 2
            try:
                selection = _resolve_agent_cmd("galph", prompt_file, selected_prompt, logp)
            except Exception as e:
                logp(f"ERROR: {e}")
                print(f"[supervisor] ERROR: {e}")
                return 2
    ...
    for _ in range(args.sync_loops):
        ...
        try:
            prompt_file, _, selected_prompt = _select_prompt(router_cmd, logp)
        except Exception as e:
            logp(f"ERROR: {e}")
